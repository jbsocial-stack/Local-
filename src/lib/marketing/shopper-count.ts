 ...
 * shoppers are already carrying a Local pass, right now. Counts
...
export function formatTractionHeadline(counts: ShopperCounts): string {
  if (counts.total === 0) {
    return 'Be one of the first shops when Local launches — only 500 passes go out per town at the start.';
  }
  ...
  return `${counts.total} shoppers are already earning points across Local towns.`;
}
