  ...
  return (
    <main className="pt-10">
      <div className="mx-auto max-w-md px-4">
        ...
        <p className="text-sm text-ink-muted">Every independent in the scheme, near you.</p>
      </div>
        ...
        <div className="pointer-events-none absolute inset-x-0 top-0 flex flex-wrap gap-2 p-3">
          <button
            ...
            className={`pointer-events-auto rounded-full px-3 py-1 text-xs font-medium shadow ${
              category === null ? 'bg-ink text-cream' : 'bg-paper/95 text-ink-muted backdrop-blur'
            }`}
          >
            All
          </button>
          {categories.map((c) => (
            <button
              ...
              className={`pointer-events-auto rounded-full px-3 py-1 text-xs font-medium shadow ${
                category === c ? 'bg-ink text-cream' : 'bg-paper/95 text-ink-muted backdrop-blur'
              }`}
            >
              {c}
            </button>
      ...
      <div className="mx-auto max-w-md px-4">
        <ul className="mt-4 divide-y divide-line rounded-[28px] bg-paper">
          {filtered.map((shop) => (
            <li key={shop.id}>
              <Link href={`/${town}/app/discover/${shop.slug}`} className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="rounded-full bg-coral-soft px-2 py-0.5 text-[11px] font-medium text-coral">
          ...
          {filtered.length === 0 && (
            <li className="px-4 py-6 text-center text-sm text-ink-muted">No shops in this category yet.</li>
          )}
        </ul>
      </div>
    </main>
